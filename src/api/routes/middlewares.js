...
module.exports.isAuthenticated = async (req, res, next) => {
   ...
   if (token) {
      try {
         ...
         return next(
            new CustomError('INVALID_ACCESS_TOKEN', 401, 'Invalid access token is supplied.'),
         )
      }
   }

   return next(new AuthenticationError(401, 'Authentication access token is not supplied.'))
}
