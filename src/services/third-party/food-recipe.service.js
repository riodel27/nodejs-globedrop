...
class FoodRecipeService {
   constructor(container) {
      ...
      this.redis_client_async_get = promisify(this.redis_client.get).bind(this.redis_client)
   }
   ...
   async getRecipesByFoodItem(foodItem) {
      ...
      if (not(recipe)) {
         ...
         this.redis_client.setex(query, 1440, JSON.stringify(response.data.results))
